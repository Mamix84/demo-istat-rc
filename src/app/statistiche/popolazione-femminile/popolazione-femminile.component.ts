...
@Component({
  selector: 'app-popolazione-femminile',
  templateUrl: './popolazione-femminile.component.html',
  styleUrls: ['./popolazione-femminile.component.scss'],
  providers: [MessageService],
})
export class PopolazioneFemminileComponent implements OnInit {
  @Input() comune: Comune;
  andamento: any;
  ultimoAnno: number;
  primoAnno: number;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    for (let i = 0; i < this.comune.dati.length; i++) {
      if (this.comune.dati[i].tipo === 'F' && this.comune.dati[i].anno > 1982) {
    ...
    datasets = [];
    datasets.push({
      label: this.comune.denominazione + ' - ' + 'F',
      data: totali,
      fill: false,
      borderColor: color,
    });
