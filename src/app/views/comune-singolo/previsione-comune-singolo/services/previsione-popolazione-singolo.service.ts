...
export class PrevisionePopolazioneSingoloService {
  constructor() {}
  ...
  calcolaPrevisione(totali: any[]): number {
    ...
    let media10anni = 0;
    let totale10anni = 0;
    for (let i = totali.length - 1; i > totali.length - 5; i--) {
      totale10anni += (totali[i] - totali[i - 1]) / totali[i];
      //console.log(totale10anni);
    }

    media10anni = totale10anni / 5;
    //console.log(media10anni);

    media10anni = 1 + media10anni;
    console.log(media10anni);
